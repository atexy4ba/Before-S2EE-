...
function SpeakerAvatar({ speaker }: { speaker: Speaker }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      {hasPhoto ? (
        <div className="w-[70px] h-[70px] md:w-[90px] md:h-[90px] overflow-hidden bg-white/5 ring-2 ring-white/10">
          <img
            alt={speaker.name}
            src={speaker.photo}
        ...
        <div className="w-[70px] h-[70px] md:w-[90px] md:h-[90px] bg-white/5 border border-white/10 flex items-center justify-center">
          <span className="font-[var(--font-heading)] text-lg md:text-xl font-bold text-white/30">
            {speaker.name
              .split(" ")
              .map((n) => n[0])
              .join("")}
      ...
      <span className="font-[var(--font-body)] text-xs md:text-sm font-semibold text-white text-center">
        {speaker.name}
      </span>
      {speaker.role && (
        <span className="font-[var(--font-body)] text-[10px] md:text-xs text-white/50 -mt-1">
          {speaker.role}
        </span>
      )}
    </div>
...
function DetailContent({ item }: { item: AgendaItem }) {
  return (
    <>
      ...
      <h4 className="font-[var(--font-heading)] text-xl md:text-[28px] font-semibold leading-[30px] md:leading-[36px] text-white">
        {item.title}
      </h4>
      <p className="font-[var(--font-body)] text-sm md:text-base leading-6 text-white/70 max-w-[480px]">
        {item.desc}
      </p>
      {item.tags && (
        <div className="flex gap-2 md:gap-3 justify-center flex-wrap">
          {item.tags.map((tag) => (
            <span
              key={tag.label}
              className={`text-[10px] md:text-xs uppercase px-3 md:px-4 py-1 md:py-1.5 font-[var(--font-body)] ${
                tag.accent
    ...
    >
      <div className="figma-mask agenda-mask absolute inset-0 bg-[#1A2478] py-20 md:py-[160px] px-[4px]">
        ...
        <div className="relative z-10 max-w-[1280px] mx-auto px-4 md:px-8 flex flex-col gap-10 md:gap-16">
          <RevealOnScroll className="mx-auto flex max-w-[768px] flex-col gap-3 md:gap-[14.8px] pb-8 md:pb-16 text-center">
          ...
          <div className="md:hidden flex flex-col gap-2">
            {agendaItems.map((item, i) => (
              <RevealOnScroll key={item.time} delay={i * 60}>
                <div
                  className={`border-l-2 pl-[36px] py-3 cursor-pointer transition-all ${
                    expandedIndex === i
                      ? "border-[#FFEDD1]"
                      : "border-white/15"
                  }`}
                ...
                >
                  <div className="flex flex-col gap-1 relative">
                    <span
                      className={`font-[var(--font-body)] text-xs md:text-sm transition-colors ${
                        expandedIndex === i
                          ? "text-[#FFEDD1]"
                          : "text-white/40"
                      }`}
                    ...
                    <h3
                      className={`font-[var(--font-heading)] text-base font-semibold leading-[22px] transition-colors ${
                        expandedIndex === i
                          ? "text-[#FFEDD1]"
                          : "text-white"
                      }`}
                    ...
                    <div
                      className={`absolute left-[-43px] top-[4px] w-3.5 h-3.5 transition-all ${
                        expandedIndex === i
                  ...
                  >
                    <div className="glass-panel-strong p-5 flex flex-col gap-5 items-center text-center">
          ...
          <div className="hidden md:grid grid-cols-12 gap-8">
            <div className="col-span-5 flex flex-col gap-3">
              {agendaItems.map((item, i) => (
                <RevealOnScroll key={item.time} delay={i * 80}>
                  <div
                    className={`relative border-l-2 pl-[40px] py-4 flex flex-col gap-1.5 cursor-pointer transition-all ${
                      i === activeIndex
                        ? "border-[#FFEDD1]"
                  ...
                  >
                    <span
                      className={`font-[var(--font-body)] text-sm transition-colors ${
                    ...
                    <h3
                      className={`font-[var(--font-heading)] text-xl font-semibold leading-[28px] transition-colors ${
                    ...
                    <div
                      className={`absolute left-[-9px] top-[22px] w-4 h-4 transition-all ${
                        i === activeIndex
            ...
            <div className="col-span-6 col-start-7">
              <RevealOnScroll delay={200} className="h-full">
                <div className="glass-panel-strong p-[32px] lg:p-[49px] flex flex-col gap-6 md:gap-8 items-center text-center relative overflow-hidden transition-all h-full justify-center min-h-[500px] lg:min-h-[600px]">
